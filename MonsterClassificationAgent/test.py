...
class test:
    ...
    def solve(self, samples, new_monster):
        ...
        for monster in samples:
            ...
            self.monster_doesnt_have['size'] = [x for x in list(self.monsterDefault['size']) if
                                                x != self.monster_has['size']]
            self.monster_doesnt_have['color'] = [x for x in list(self.monsterDefault['color']) if
                                                 x != self.monster_has['color']]
            self.monster_doesnt_have['covering'] = [x for x in list(self.monsterDefault['covering']) if
                                                    x != self.monster_has['covering']]
            self.monster_doesnt_have['foot-type'] = [x for x in list(self.monsterDefault['foot-type']) if
                                                     x != self.monster_has['foot-type']]
            self.monster_doesnt_have['leg-count'] = [x for x in list(self.monsterDefault['leg-count']) if
                                                     x != self.monster_has['leg-count']]
            self.monster_doesnt_have['arm-count'] = [x for x in list(self.monsterDefault['arm-count']) if
                                                     x != self.monster_has['arm-count']]
            self.monster_doesnt_have['eye-count'] = [x for x in list(self.monsterDefault['eye-count']) if
                                                     x != self.monster_has['eye-count']]
            self.monster_doesnt_have['horn-count'] = [x for x in list(self.monsterDefault['horn-count']) if
                                                      x != self.monster_has['horn-count']]
            self.monster_doesnt_have['lays-eggs'] = [x for x in list(self.monsterDefault['lays-eggs']) if
                                                     x != self.monster_has['lays-eggs']]
            self.monster_doesnt_have['has-wings'] = [x for x in list(self.monsterDefault['has-wings']) if
                                                     x != self.monster_has['has-wings']]
            self.monster_doesnt_have['has-gills'] = [x for x in list(self.monsterDefault['has-gills']) if
                                                     x != self.monster_has['has-gills']]
            self.monster_doesnt_have['has-tail'] = [x for x in list(self.monsterDefault['has-tail']) if
                                                    x != self.monster_has['has-tail']]
